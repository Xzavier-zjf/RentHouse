...
```text
rent-app/user-service/src/main/resources/application.yml
rent-app/house-service/src/main/resources/application.yml
```
...
```bash
mysql -uroot -p123456 < rent-app/db-init/init.sql
```

### 旧库升级

如果已有旧版 `rent_db`，在仓库根目录执行：

```bash
mysql -uroot -p123456 rent_db -e "source D:/IdeaProjects/RentHouse/rent-app/db-init/upgrade-enhanced.sql;"
```
...
```bash
cd rent-app/user-service
mvn spring-boot:run
```

```bash
cd rent-app/house-service
mvn spring-boot:run
```
...
- 请使用 `http://localhost:3000` 访问 Vue 前端，不要打开 `rent-app/index.html`，旧静态页面不会展示最新功能。
...
- [需求分析说明书](rent-app/docs/需求分析说明书.md)
- [开发文档](rent-app/docs/开发文档.md)
- [rent-app 内部 README](rent-app/README.md)
