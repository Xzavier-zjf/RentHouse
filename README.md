...
如果已有旧版数据库，可在 `rent-app` 目录执行增强版升级脚本：
...
后端服务可分别在 `rent-app/user-service`、`rent-app/house-service`、`rent-app/gateway` 中执行 Maven 编译：

```bash
mvn -q -DskipTests compile
```

前端在 `rent-app/vue-frontend` 中执行：
